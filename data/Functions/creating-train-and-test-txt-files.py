...
for current_dir, dirs, files in os.walk('.'):
    ...
    for f in files:
        ...
        if f.endswith('.jpg'):
            print('f: ', f.split('.')[0]+'.txt')
            with open(f.split('.')[0]+'.txt', 'r+') as draft_txt:
                # Going through all elements of the list
                cont = draft_txt.read().split('\n')[:-1]
                cont = [i.split(' ') for i in cont]
                anno = full_path_to_images + '/'+ f + ' '
                for idx, val in enumerate(cont):
                    if idx != len(cont)-1:
                        anno = anno + ' ' + val[1] + ',' + val[2] + ',' + val[3] + ',' + val[4]
                    else:
                        anno = anno + ' ' + val[1] + ',' + val[2] + ',' + val[3] + ',' + val[4] + '\n'
                print('draft_txt: ', cont)
                print('draft_txt: ', anno)
